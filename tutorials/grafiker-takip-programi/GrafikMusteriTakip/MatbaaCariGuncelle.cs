...
{
    public partial class MatbaaCariGuncelle : Form
    {
        public MatbaaCariGuncelle()
        {
            InitializeComponent();
        }
        private void GetirBilgi()
        {
            OleDbCommand cmd = new OleDbCommand("SELECT * FROM Matbaa WHERE ((Matbaa.MatbaaID)=@ID)", con);
        ...
        private void Ekle_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult dlg = MessageBox.Show("Matbaa Bilgileri Güncellensin mi?", "Güncellenme Onay", MessageBoxButtons.YesNo, MessageBoxIcon.Information);

                if (dlg == DialogResult.Yes)
                {
                    OleDbCommand comcuk = new OleDbCommand("Update Matbaa set Ad=@A,Unvan=@U,Yetkili=@Y,Cep=@C,Telefon=@T,Faks=@F,Posta=@P,Adres=@Ad,VDairesi=@VD,VergiNo=@VN where ((Matbaa.MatbaaID)=@ID)", con);
        ...
        private void MusteriGuncelle_Load(object sender, EventArgs e)
        {
            ...
            GelenID = Convert.ToInt32(MatbaaCari.GidenID);
            GetirBilgi();
        }
        ...
        {
            DialogResult dlg = MessageBox.Show("Matbaa Bilgileri Silinsinn mi?", "Silme Onay", MessageBoxButtons.YesNo, MessageBoxIcon.Information);

            if (dlg == DialogResult.Yes)
            {
                OleDbCommand comcuk = new OleDbCommand("Update Matbaa set IsActive=0 where ((Matbaa.MatbaaID)=@ID)", con);
                comcuk.Parameters.AddWithValue("@ID", GelenID);
                if (con.State == ConnectionState.Closed)
                {
                    con.Open();
